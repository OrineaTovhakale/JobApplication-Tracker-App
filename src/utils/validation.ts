import { VALIDATION, JOB_STATUSES } from '../constants';
...
export const validateJob = (job: Partial<Job>): { valid: boolean; error?: string } => {
  if (!job.company?.trim()) return { valid: false, error: 'Company name is required' };
  ...
  if (!job.role?.trim()) return { valid: false, error: 'Job role is required' };
  ...
  if (!job.dateApplied) return { valid: false, error: 'Date applied is required' };
  if (job.status && !JOB_STATUSES.includes(job.status as Job['status']))
